...
- **`.eleventy.js`**: Core SSG configuration - defines collections, passthrough copies, and custom filters. Manages the "sortedpost" collection sorted by date (newest first).
- **`_includes/`**: Nunjucks layout templates. `layout.njk` is the base HTML structure; `post.njk` extends it for individual post pages.
- **`posts/`**: Markdown content files. Each post is in its own folder (e.g., `posts/post-slug/index.md`) with a separate `img/` subfolder for images.
- **`styles/`**: CSS files. `reset.css` for normalization; `custom.css` for styling. Passthrough copied to output.
- **`index.njk`**: Homepage template that iterates over `collections.sortedpost` to display post previews.
...
- **Images**: Store in `posts/{post-slug}/img/` and reference locally: `![alt text](./img/filename.png)`
...
Eleventy generates files in `_site/` (configured in `.eleventy.js`). Directory structure mirrors input, so:
- `posts/post-slug/index.md` → `_site/posts/post-slug/index.html`
- Images in `posts/post-slug/img/` → `_site/posts/post-slug/img/`
...
**Adding a new post**: Create `posts/{slug}/index.md` with proper frontmatter, `tags: post`, and `layout: post`.

**Modifying templates**: Edit `.njk` files in `_includes/` or `index.njk`. Eleventy hot-reloads on save in dev mode.

**Image placement**: Store images alongside content in `posts/{slug}/img/`. They're auto-passthrough-copied to output.

**Updating styles**: Modify `styles/custom.css`. Changes apply to all pages via `layout.njk`.
