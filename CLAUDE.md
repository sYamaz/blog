...
- **`@11ty/eleventy-plugin-rss`**: RSSフィード生成プラグイン（設定済み）。
...
- **全角・半角アルファベット混在禁止**（`.textlintrc`）: 同一文書内で全角と半角アルファベットを混在させない。
...
**新規記事追加**: `mise run new [slug] [title]`を実行（引数省略時はインタラクティブ入力）。フォルダ・frontmatter・画像ディレクトリを自動生成しエディタを起動する。手動で作成する場合は`src/posts/{slug}/index.md`を適切なフロントマター（`tags: post`, `layout: post`）で作成。
